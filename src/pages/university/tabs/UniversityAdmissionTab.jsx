...
const UniversityAdmissionTab = () => {
  ...
  const fetchData = useCallback(async () => {
    setLoading(true);
    try {
      const data = await universityViewService.getMyAdmissionMethods();
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editingMethod) {
        await universityViewService.updateMyAdmissionMethod(editingMethod.id, methodData);
        toast.success('Cập nhật phương thức tuyển sinh thành công!');
      } else {
        await universityViewService.createMyAdmissionMethod(methodData);
